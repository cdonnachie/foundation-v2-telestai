...
// Verify constants:
static_assert(sizeof(ethash::hash512) == MERAKI_LIGHT_CACHE_ITEM_SIZE, "");
static_assert(sizeof(ethash::hash1024) == MERAKI_FULL_DATASET_ITEM_SIZE, "");
    ...
    build_light_cache_fn build_fn, int epoch_number, bool full) noexcept
{
    ...
    for (uint32_t i = 0; i < meraki_progpow::l1_cache_size / sizeof(full_dataset_2048[0]); ++i)
...
using namespace meraki_main;

extern "C" {

ethash_hash256 meraki_calculate_epoch_seed(int epoch_number) noexcept
{
...
bool meraki_verify_final_hash(const ethash::hash256* header_hash, const ethash::hash256* mix_hash, uint64_t nonce,
...
bool meraki_verify(const epoch_context* context, const ethash::hash256* header_hash,
