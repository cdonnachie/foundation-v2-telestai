...
const Template = function(jobId, config, rpcData, placeholder) {
  ...
  this.difficulty = parseFloat((Algorithms.meraki.diff / Number(_this.target)).toFixed(9));
  ...
  this.handleParameters = function(client, cleanJobs) {
    ...
    const epochLength = Math.floor(this.rpcData.height / Algorithms.meraki.epochLength);
    const extraNonce1Buffer = Buffer.from(client.extraNonce1, 'hex');

    // Pad Difficulty to Broadcast
    let zeroPad = '';
    const adjPow = Algorithms.meraki.diff / _this.difficulty;
